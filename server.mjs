...
import QuickAddItem from "./models/QuickAddItem.js";
...
const mongoUri = `mongodb+srv://${DB_USERNAME}:${encodeURIComponent(DB_PASSWORD)}@${DB_CLUSTER}/${DB_NAME}?retryWrites=true&w=majority`;
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updated = await Category.findOneAndUpdate({ name, user: req.userId }, { name: newName }, { new: true });
    if (!updated) {
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updated = await PaymentSource.findOneAndUpdate({ name, user: req.userId }, { name: newName }, { new: true });
    if (!updated) {
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updated = await QuickAddItem.findOneAndUpdate({ _id: id, user: req.userId }, updates, { new: true });
...
app.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const deleted = await QuickAddItem.findOneAndDelete({ _id: id, user: req.userId });
...
app.post(
  ...
  validateRestore,
  asyncHandler(async (req, res) => {
    const { expenses = [], categories = [], paymentSources = [], quickAddItems = [] } = req.body || {};
...
const server = app.listen(PORT, () => {
  logger.info(`Server running on port ${PORT}`, { environment: process.env.NODE_ENV || "development", port: PORT });
});
